...
## 总结
程序优化了几个星期，程序上几乎没有优化的余地，唯一可以优化的就是，别让程序跳多个文件，因为我的项目是基于pystudy
的，所以，上面跳了很多次，不知道这里有没有影响。所以，有点想优化下PYSTUDY。

异步的mysql库是没办法弄的，我这同步的库是唯一可以和tornado能对接的，tormysql无法与tornado一起使用。

1G1核，还做了这么多分布，草泥马的才50左右的tps，并且只有100个线程的并发。

我现在有一个优化的方法是，客户端将数据请求发送过来之后，服务端接到之后，先不处理，先马上告诉客户端我已经收到了，
然后再处理，但是，这样出现的问题就是，mysql这边崩溃了，因为，大量的操作来了，mysql连接池也要有空闲的才能处理，
那么，还是没有办法。

主从，读从，写主，redis由于是内存基本可以不耗时，amqp由于是起的进程服务，所以，基本不会影响tornado，那么，到底
是什么影响了我的tornado的吞吐量呢？linux的并发参数我也改了，现在只能支持100个线程。

当我把连接池的大小设置为100后，奇迹出现了，100个线程的吞吐量为71，增加了20多个。

所以，可以从线程池的实现那里优化和增加mysql连接限制的优化.

500个线程的时候，吞吐量为90,有0.12%的错误

最终改为100个线程时，吞吐量稳定到85左右，所以，优化了这么多天的结果就是：

# tornado的并发的关键就是mysql
